...
class Model(nn.Module):
    ...
    llm_model: str = "gemma_bv"
    llm: Optional[ConfigDict] = None

    def setup(self):
        # TODO: I don't love this pattern... change to importing like normal.
        self._llm = importlib.import_module(
            f"pi_zero_project.model.vlm.llm_model.{self.llm_model}"
        ).Model(**(self.llm or {}), name="llm")

        img_config = {"num_classes": self._llm.embdim, **(self.img or {})}
        self._img_model = importlib.import_module(
            f"pi_zero_project.model.vlm.img_model.{self.img_model}"
    ...
    def prefill_cache(self, x, input_mask, mask_ar, *, cache_size):
        ...
        if hasattr(self._llm, "prefill_cache"):
            ...
            return self._llm.prefill_cache(x, input_mask, attn_mask, cache_size=cache_size)
        else:
    ...
    def _fallback_prefill_cache(self, x, input_mask, mask_ar, cache_size):
        ...
        x = jnp.pad(jnp.where(input_mask[..., None], x, 0), [(0, 0), (0, pad_size), (0, 0)])  # type: ignore
        mask_ar = jnp.pad(jnp.where(input_mask, mask_ar, 0), [(0, 0), (0, pad_size)])  # type: ignore
    ...
    def _fallback_extend_cache(self, x):
        ...
        input_mask = jnp.logical_or(c_input_mask, jnp.einsum("be,ben->bn", input_mask, move_onehot))
        self.put_variable("cache", "x_cache", x)
        self.put_variable("cache", "input_mask_cache", input_mask)
        self.put_variable("cache", "mask_ar_cache", mask_ar)
...
def load(
    init_params, init_files, model_cfg, img_load_kw={}, llm_load_kw={}
