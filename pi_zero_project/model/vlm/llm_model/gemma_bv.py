...
from pi_zero_project.model.vlm.llm_model import gemma
...
class Model(nn.Module):
    ...
    def setup(self):
        ...
        self.model = gemma.Model(**_get_config(self), parent=self.scope, name="")  # type: ignore
    ...
    def prefill_cache(self, x, input_mask, attn_mask, *, cache_size):
        ...
        self.put_variable("cache", "cache_end", jnp.full((batch_size,), prefill_len, jnp.int32))
    ...
    def extend_cache(self, x):
        ...
        logits, _ = self.model(
            tokens=None, embedded_prefix=x, positions=positions[:, None], mask=mask, decode=True
        )
        return logits
