...
class ViT(nn.Module, BaseImageModel):
    ...
    def __call__(self, image, *, train=False):
        ...
        n, h, w, c = x.shape
        x = jnp.reshape(x, [n, h * w, c])  # [N, L, D] s.t. L = H * W
        ...
        if self.pool_type == "tok":
            ...
            x = jnp.concatenate([jnp.tile(cls, [n, 1, 1]), x], axis=1)  # [N, L+1, D]

        n, l, c = x.shape  # pylint: disable=unused-variable
        x = nn.Dropout(rate=self.dropout)(x, not train)  # [N, L+1, D]
        ...
        encoded = out["encoded"] = x  # [N, L+1, D]

        if self.pool_type == "map":
            x = out["head_input"] = MAPHead(num_heads=self.num_heads, mlp_dim=self.mlp_dim)(x)
        ...
        x_2d = jnp.reshape(encoded, [n, h, w, -1])  # [N, H, W, D]

        if self.rep_size:  # let D now equal new representation size
            rep_size = self.width if self.rep_size is True else self.rep_size
            hid = nn.Dense(rep_size, name="pre_logits")
            # NOTE: In the past we did not include tanh in pre_logits.
            # For few-shot, it should not matter much, as it whitens anyways.
            x_2d = nn.tanh(hid(x_2d))  # [N, H, W, D]
            x = nn.tanh(hid(x))  # [N, D]

        out["pre_logits_2d"] = x_2d  # [N, H, W, D]
        out["pre_logits"] = x  # [N, D]

        if self.num_classes:
            kw = {"kernel_init": nn.initializers.zeros} if self.head_zeroinit else {}
            head = nn.Dense(self.num_classes, name="head", **kw)  # type: ignore
            x_2d = out["logits_2d"] = head(x_2d)  # [N, H, W, K] s.t. K = num_classes
            x = out["logits"] = head(x)  # [N, K]
...
def fix_old_checkpoints(params):
    ...
    params = flax.core.unfreeze(flax.training.checkpoints.convert_pre_linen(params))  # type: ignore
    ...
    if "probe" in params:
        params["MAPHead_0"] = {
            k: params.pop(k)
            for k in ["probe", "MlpBlock_0", "MultiHeadDotProductAttention_0", "LayerNorm_0"]
        }
...
def pyloop_to_scan(params_pyloop):
    ...
    t["encoderblock"] = jax.tree.map(stack, *[t[f"encoderblock_{lyr}"] for lyr in range(depth)])
...
def load(
    ...
    restored_params = u.merge_params(restored_params, init_params, dont_load)
