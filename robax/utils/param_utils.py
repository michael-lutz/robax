# https://github.com/google-research/big_vision/blob/main/big_vision/models/common.py
    ...
    def pad_shard_unpad_wrapper(*args, min_device_batch=None, **kw):
        ...
        def pad(x):
            ...
            if min_device_batch and db < min_device_batch:
                x = np.concatenate([x, np.zeros((d * (min_device_batch - db), *shape), x.dtype)])
                db = min_device_batch
...
def weighted_softmax_xent(
    *, logits, labels, reduction=True, weights=None, label_smoothing=0.0, normalize=True
    ...
    soft_targets = onehot(labels, vocab_size, on_value=confidence, off_value=low_confidence)
...
def accumulate_gradient(loss_and_grad_fn, params, images, labels, accum_steps):
    ...
    if accum_steps and accum_steps > 1:
        ...
        def acc_grad_and_loss(i, l_and_g):
            ...
            lbls = jax.lax.dynamic_slice(labels, (i * step_size, 0), (step_size, labels.shape[1]))
...
def itstime(
    step, every_n_steps, total_steps, host=None, last=True, first=True, drop_close_to_last=0.25
):
...
class Chrono:
    ...
    def log_timing_avg(self, name, *, noop=False):
        ...
        if not noop:
            ...
            logging.info(
                "TIMING[%s]: avg %s current %s", name, np.mean(self._timing_history[name]), dt
            )
            logging.flush()
        ...
        for pattern, arg in regex_rules:
            if re.fullmatch(pattern, vname):
                if name and jax.process_index() == 0:
                    logging.info("Applying %s to %s with %s due to `%s`", name, vname, arg, pattern)
                return f(v, arg)
        return not_f(v)
...
def tree_replace(tree, replacements):
    ...
    names_and_vals = [(rename(k), v) for k, v in names_and_vals if not should_remove(k)]
...
def tree_filter(tree, mask):
    ...
    return {k: tree_filter(v, mask[k]) for k, v in tree.items() if mask[k] is not False}
    ...
    def _on_commit_callback(array_names):  # Runs after writing ckpt is done.
        ...
        if last.endswith("-tmp"):
            ...
            multiprocessing.pool.ThreadPool().map(
                gfile.rmtree, [f"{path}-{last}/{name}" for name in array_names]
            )
...
def load_checkpoint_ts(path, **tsload_kw):
    ...
    try:
        ...
        Exception
    ):  # Differs based on backend, so blanket catch. pylint:disable=broad-exception-caught
        pass
...
def tsload(path, *, tree=None, shardings=None, regex=None):
    ...
    if tree is None:
        ...
        path_names = set([p.rstrip("/").replace("~", "/") for p in gfile.listdir(path)])
    ...
    if shardings is None:
        shardings = jax.sharding.SingleDeviceSharding(jax.local_devices(backend="cpu")[0])
    shardings = list(jax.tree.leaves(tree_broadcast(shardings, tree)))

    names_to_load = [os.path.join(path, name.replace("/", "~")) for name in names_to_load]
...
def create_learning_rate_schedule(
    ...
    **kw,
    ...
    assert (total_steps <= 1) or (warmup_steps < total_steps), "warmup_steps is >= total_steps"
...
def sync():
    ...
    x = reshard(np.ones(jax.device_count()), jax.sharding.PositionalSharding(jax.devices()))
...
def make_mask_trees(tree, patterns, *, log=None):
    ...
    def matchfirst(name, _):
        ...
        if log is not None and True in matches and jax.process_index() == 0:
            logging.info("%s: %s - matched by %s", log, name, patterns[matches.index(True)])
        return np.array(matches)

    multimask = tree_map_with_names(matchfirst, tree)
    return [
        jax.tree.map(lambda matches, i=idx: matches[i], multimask) for idx in range(len(patterns))
    ]
...
def startstop_prof(sess, step=None, first_step=0, log_steps=1, surround=10, **kw):
...
class BigVisionMetricWriter:
    ...
    def __init__(self, xid=-1, wid=-1, workdir=None, config=None):
        ...
        if workdir:
            if xid != -1 and wid != -1:
                self.fname = os.path.join(workdir, f"big_vision_{xid}_{wid}_metrics.txt")
            else:
...
def assert_local_slices_same(*global_arrays):
    ...
    slices = [
        tuple(
            tuple((idx.start, idx.end, idx.step) for idx in s.index) for s in a.addressable_shards
        )
...
def jit_cpu(**extra_kwargs):
    def _decorator(fun):
        def _wrapped(*args, **kwargs):
            sh = jax.sharding.SingleDeviceSharding(jax.local_devices(backend="cpu")[0])
